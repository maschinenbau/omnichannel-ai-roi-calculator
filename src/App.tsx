...
// Updated RangeSlider component with an adjacent number input and tooltip for helpText
const RangeSlider = ({ label, id, value, onChange, min, max, step, unit = '', helpText = '' }) => {
  ...
  // Determine display formatting based on unit and step
  ...
  return (
    // Increased bottom margin for more space between sliders
    <div className="mb-5"> 
      <div className="flex justify-between items-center mb-1">
        <div className="flex items-center flex-1"> 
          ...
          {helpText && (
            <div className="relative flex items-center ml-2 group cursor-pointer">
              ...
              {/* Tooltip content: appears on hover of the group */}
        ...
        {/* Container for the number input and its internal unit symbol */}
        <div className="relative flex items-center ml-2"> 
          <input 
            ...
            max={max}
            step={step}
            // Adjusted width (w-24 from w-28) and padding
          ...
          />
          {/* Unit symbol positioned inside the input */}
          {unit.includes('$') && (
...
const LabelWithTooltip = ({ label, tooltipText, labelClasses = "text-md font-semibold text-gray-700", valueClasses = "" }) => (
    <div className="flex items-center justify-between w-full"> {/* Ensure full width for proper alignment */}
      <div className="flex items-center"> {/* Group label and tooltip icon */}
...
// PrintStyles component for print-specific CSS
const PrintStyles = () => (
  <style type="text/css">
    {`
      @media print {
        ...
        /* Tooltips should not appear in print */
...
// safeLocaleString helper function to format numbers
...
function OmnichannelAiRoiCalculator() {
  ...
  const [avgRevenuePerSale, setAvgRevenuePerSale] = useState(industryPresets.general.avgRevenuePerSale);
  const [operatingDaysPerMonth, setOperatingDaysPerMonth] = useState(industryPresets.general.operatingDaysPerMonth);
  const [monthlyVoiceCalls, setMonthlyVoiceCalls] = useState(industryPresets.general.monthlyVoiceCalls);
  const [avgMissedVoiceCallsDaily, setAvgMissedVoiceCallsDaily] = useState(industryPresets.general.avgMissedVoiceCallsDaily);
  const [avgCallDurationHuman, setAvgCallDurationHuman] = useState(industryPresets.general.avgCallDurationHuman);
  const [monthlyTextInteractions, setMonthlyTextInteractions] = useState(industryPresets.general.monthlyTextInteractions);
  const [avgTextInteractionTimeHuman, setAvgTextInteractionTimeHuman] = useState(industryPresets.general.avgTextInteractionTimeHuman);
  const [humanHourlyCost, setHumanHourlyCost] = useState(industryPresets.general.humanHourlyCost);
  const [avgMessagesPerTextConversation, setAvgMessagesPerTextConversation] = useState(industryPresets.general.avgMessagesPerTextConversation); 

  const [voiceLeadQualificationRate, setVoiceLeadQualificationRate] = useState(industryPresets.general.voiceLeadQualificationRate);
  const [voiceAppointmentBookingRate, setVoiceAppointmentBookingRate] = useState(industryPresets.general.voiceAppointmentBookingRate);
  const [voiceAppointmentShowUpRate, setVoiceAppointmentShowUpRate] = useState(industryPresets.general.voiceAppointmentShowUpRate);
  const [voiceAppointmentToSaleRate, setVoiceAppointmentToSaleRate] = useState(industryPresets.general.voiceAppointmentToSaleRate);
  const [textLeadQualificationRate, setTextLeadQualificationRate] = useState(industryPresets.general.textLeadQualificationRate);
  const [textAppointmentBookingRate, setTextAppointmentBookingRate] = useState(industryPresets.general.textAppointmentBookingRate);
  const [textAppointmentShowUpRate, setTextAppointmentShowUpRate] = useState(industryPresets.general.textAppointmentShowUpRate);
  const [textAppointmentToSaleRate, setTextAppointmentToSaleRate] = useState(industryPresets.general.textAppointmentToSaleRate);
  const [aiTier, setAiTier] = useState('basic');
  const [aiHandlingPercentage, setAiHandlingPercentage] = useState(industryPresets.general.aiHandlingPercentage);
  
  const [basicAiSetupFee, setBasicAiSetupFee] = useState(2000); 
  const [basicAiMonthlyCost, setBasicAiMonthlyCost] = useState(497); 
  const [basicAiPerMinuteVoiceCost, setBasicAiPerMinuteVoiceCost] = useState(0.45); 
  const [basicAiIncludedTextInteractions, setBasicAiIncludedTextInteractions] = useState(1000); 
  const [basicAiOverageTextCost, setBasicAiOverageTextCost] = useState(0.05); 

  const [enterpriseAiSetupFee, setEnterpriseAiSetupFee] = useState(5000); 
  const [enterpriseAiMonthlyCost, setEnterpriseAiMonthlyCost] = useState(2000); 
  const [enterpriseAiPerMinuteVoiceCost, setEnterpriseAiPerMinuteVoiceCost] = useState(0.30); 
  const [enterpriseAiIncludedTextInteractions, setEnterpriseAiIncludedTextInteractions] = useState(10000); 
  const [enterpriseAiOverageTextCost, setEnterpriseAiOverageTextCost] = useState(0.03); 

  const [aiAutonomyVoice, setAiAutonomyVoice] = useState(industryPresets.general.aiAutonomyVoice);
  const [aiAutonomyText, setAiAutonomyText] = useState(industryPresets.general.aiAutonomyText);
  const [aiVoiceBookingRateImprovement, setAiVoiceBookingRateImprovement] = useState(10); 
  const [aiVoiceShowUpRateImprovement, setAiVoiceShowUpRateImprovement] = useState(10); 
  const [aiTextBookingRateImprovement, setAiTextBookingRateImprovement] = useState(10); 
  const [aiTextShowUpRateImprovement, setAiTextShowUpRateImprovement] = useState(10); 
 ...
 useEffect(() => {
  ...
  const getScenarioInterpretation = () => {
    if (!results.aiHandlingPercentage && results.aiHandlingPercentage !== 0) return "Adjust the 'AI Handling Percentage' to model different levels of automation.";
    if (results.aiHandlingPercentage === 100) {
    ...
    return `Enhancement Focus: AI handles ${results.aiHandlingPercentage}% of interactions. Human agents manage the remaining ${100 - results.aiHandlingPercentage}%, likely complex cases. Savings from increased efficiency.`;
  };
